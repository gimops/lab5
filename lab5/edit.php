		...
		<?
			if(isset($_GET['id'])&&isset($_GET['query'])){
                    ...
                    while ($row=mysqli_fetch_array($result)){
                        for($i = 0; $i < count($row)/2; $i++){
                            if($i == 0){
                                ...
                                $ar =  $row[$i];
                                print "Изменить $arrayTitle[$i]: <input name='$array[$i]' size='50' type='text' value='$ar' maxlength='128'> <br>";
                            }
                        }
                    }
                    echo("<input type='hidden' name='index' value='$index'> <br>");
                ...
                case "dk_info":
                    ...
                    echo("Введите стоимость: <input name='cost' type='number'min='999' max='999999' value='$rows[5]' /> <br>");
